...
public class OrderedLinearProbingHashTable extends OpenAddressingHashTable {
    ...
    public Probes put(String key, String value) {
    	...
    	if (resize < ((double) this.count)/(double)this.table.length) {
    		...
    		if (record.size() > 0) {
    			for (KVPair p : record) {
    				num_probes += put(p.getKey(), p.getValue()).getProbes();
    			}
    		}
    	}
    	...
    	while(!(this.table[hashIdx] == null)) {
    		...
    		if (this.table[hashIdx].getKey().compareTo(new_pair.getKey()) > 0 && this.table[hashIdx] != this.TOMBSTONE) {
    			KVPair temp_pair = this.table[hashIdx];
    			this.table[hashIdx] = new_pair;
    			new_pair = temp_pair;
    		}else {
    ...
    public Probes get(String key) {
        ...
        while(this.table[hashIdx] !=null) {
        	if(this.table[hashIdx] != this.TOMBSTONE) {
        		if (key.compareTo(this.table[hashIdx].getKey())==0) {
        			return new Probes(table[hashIdx].getValue(), probes);
        		}else if (key.compareTo(this.table[hashIdx].getKey())<0) {
        			// this means that we exceeded the proper place for the key to be at
    ...
    public static void main(String[] args) {
    	OrderedLinearProbingHashTable ol = new OrderedLinearProbingHashTable(true);
    	ol.put("DeAndre", "a");
    	ol.put("Charles", "b");
    	ol.put("Christine", "c");
    	ol.put("Alexander", "d");
